...
public class LinnStrumentExtension extends ControllerExtension
{
   ...
   public void init()
   {
      ...
      sendNRPN(0, 245, 1 << 7);  // ENTER USER FIRMWARE MODE
   ...
   private void onTimer()
   {
      ...
      for(int r=0; r<8; r++)
      {
         setLED(row, column, r == row ? color : 0);
      }

      mCount++;

      getHost().scheduleTask(this::onTimer, 200);
   }
   ...
   void setLED(int row, int column, int color)
   {
      ...
      midiOut.sendMidi(ShortMidiMessage.CONTROL_CHANGE, 20, row);
      midiOut.sendMidi(ShortMidiMessage.CONTROL_CHANGE, 21, column);
